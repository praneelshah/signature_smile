...
const Home = () => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const progress = Math.min(
        Math.max((scrollY - sectionTop) / scrollRange, 0),
        1
      );
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const targetTime = HERO_VIDEO_START + targetTimeRef.current;
      const delta = targetTime - currentTime;

      if (Math.abs(delta) > 0.001) {
        const nextTime = currentTime + delta * SMOOTHING_FACTOR;
        heroVideoRef.current.currentTime = Math.min(
          HERO_VIDEO_END,
          Math.max(HERO_VIDEO_START, nextTime)
        );
      }
  ...
  const handleLoadedMetadata = () => {
    if (heroVideoRef.current) {
      ...
      const effectiveStart = Math.min(HERO_VIDEO_START, duration);
      const effectiveEnd = Math.min(HERO_VIDEO_END, duration);
      const trimmedDuration = Math.max(effectiveEnd - effectiveStart, 0);
